...
) -> tuple[GlueContext, SparkSession]:
    conf_list = [
        ...
        (
            ...
            f"s3://{warehouse_bucket}/{catalog_name}/",
        ),
        (
...
def read_data(spark: SparkSession) -> DataFrame:
    ...
    return spark.createDataFrame(
        data=[(1, "Alice", 28), (2, "Bob", 34), (3, "Charlie", 23)],
        schema=["id", "name", "age"],
    )
...
def wap_append(
    ...
    try:
        ...
        spark.sql(
            f"ALTER TABLE {full_table_name} SET TBLPROPERTIES ('write.wap.enabled'='true')"
        )
        ...
        spark.sql(
            f"ALTER TABLE {full_table_name} DROP BRANCH IF EXISTS {audit_branch_name}"
        )
        spark.sql(f"ALTER TABLE {full_table_name} CREATE BRANCH {audit_branch_name}")
        (
            data_df.write.format("iceberg")
            .mode("append")
        ...
        if audit_passed:
            ...
            spark.sql(
                f"CALL {catalog_name}.system.fast_forward('{full_table_name}', 'main', '{audit_branch_name}')"
            )
